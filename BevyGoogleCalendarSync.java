...
@Command(name = "BevyGoogleCalendarSync", mixinStandardHelpOptions = true, version = "BevyGoogleCalendarSync 0.2",
        description = "BevyGoogleCalendarSync made with jbang", defaultValueProvider = PropertiesDefaultProvider.class)
class BevyGoogleCalendarSync implements Callable<Integer> {

    private static final String APPLICATION_NAME = "BevyGoogleCalendarSync by Manik";
    ...
    @Parameters(index = "0", description = "Properties Path")
    private File propertiesPath;


    @Option(names = {"-cf", "--credentialsFile"}, description = "Credentials file", defaultValue = "./.local/credentials.json")
    private File credentialsFile;

    @Option(names = {"-cid", "--calenderId"}, description = "Google Calendar Id")
    private String calendarId;

    @Option(names = {"-b", "--bevyUrl"}, description = "Bevy events url")
    private String bevyUrl;

    @Option(names = {"-ep", "--eventPrefix"}, description = "Event Id prefix")
    private String eventPrefix;

    public static void main(String... args) {
        String configPath = args[0];
        CommandLine cmd = new CommandLine(new BevyGoogleCalendarSync());
        File defaultsFile = new File(configPath);
    ...
    public Integer call() throws Exception {
        if(eventPrefix == null) {
            eventPrefix = calendarId.split("@")[0];
        } else {
            System.out.println("Using provided event prefix '" + eventPrefix + "'. " +
                    "It should never be changed for calendar id '" + calendarId + "'. " +
                    "Otherwise, duplicate events may get added to calendar for next few weeks.");
        }
        System.out.println("######################## \nExecution parameters: propertiesPath: " + propertiesPath +", \n" +
        ...
        for (MeetupEvent meetupEvent: upcomingEvents) {
            ...
            if(calendarEvents.containsKey(eventId)) {
                ...
                try{
                    ...
                    inserted = service.events().insert(calendarId, toInsert).execute();
    ...
    private Event updateEvent(Calendar service, MeetupEvent meetupEvent, String eventId, Event toInsert) throws IOException {
        ...
        inserted = service.events().update(calendarId, eventId, toInsert).execute();
        return inserted;
    }
